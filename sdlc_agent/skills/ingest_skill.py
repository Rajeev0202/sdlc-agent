...
class IngestSkillAutomation:
        ...
        if source.startswith("http://") or source.startswith("https://"):
            ...
            if "confluence" in source.lower() or "atlassian" in source.lower():
                ...
                try:
                    ...
                    raise ValueError(
                        f"Failed to fetch Confluence page: {str(e)}\n\n"
    ...
    def _parse_requirements(self, content: str, source: str) -> dict[str, Any]:
        ...
        Tries LLM-based extraction first (handles any format), then falls
        back to regex-based extraction for "As a / I want / so that" patterns.
        """
        ...
        # Try LLM-based extraction first (much more flexible than regex)
        if self.llm.is_live and len(cleaned.strip()) > 50:
            ...
            if llm_result and llm_result.get("stories"):
                print(f"[Ingest Parser] LLM extracted {len(llm_result['stories'])} stories, "
                      f"{len(llm_result.get('acceptance_criteria', []))} ACs")
                return llm_result
        ...
        # Extract user stories - support multiple formats:
        ...
        print(f"[Ingest Parser] Found {len(result['stories'])} user stories")
...
Return only the JSON object."""
    ...
    def _clean_markdown(self, content: str) -> str:
        """Strip markdown formatting that interferes with regex matching."""
