...
test.describe('Story US-008: ', () => {
  test('AC1: Given an unfreeze action occurs, when it completes, then audit log con', async ({ page }) => {
    ...
    // Assert: Verify Given an unfreeze action occurs, when it completes
  ...
  test('AC3: Given failed auth attempts, when logged, then audit trail captures all', async ({ page }) => {
    // Arrange: Navigate to application
    await page.goto('http://localhost:3000');
    await page.waitForLoadState('networkidle');
    ...
    // Assert: Verify Given failed auth attempts, when logged, then audi
    const statusMessage = page.locator('[data-testid="status-message"]');
    await expect(statusMessage).toBeVisible();
    await expect(statusMessage).toContainText(/success|complete/i);
  });
