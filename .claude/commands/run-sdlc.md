...
1. Generate a run id (`run-YYYYMMDD-HHMMSS`) and create `runs/<run-id>/`.
2. Invoke the `stage1-requirement` subagent with the BRD path. Save its
   output to `runs/<run-id>/01_brief.json`.
3. Invoke the `stage2-stories` subagent. Save to `runs/<run-id>/02_backlog.json`.
4. **STOP.** Print the backlog as a table and tell the user to run
   `/approve-backlog runs/<run-id>` to proceed.
